...
void calculateOccupancyBACKWARD(int threadsPerBlock, size_t sharedMemBytes, cudaFuncCache cacheConfig) {
    cudaDeviceProp prop;
    cudaGetDeviceProperties(&prop, 0);
    ...
    // Set function attributes
    cudaFuncSetCacheConfig(backward_kernel, cacheConfig);
    
    // Calculate occupancy
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocks, backward_kernel, threadsPerBlock, sharedMemBytes);
    activeWarps = numBlocks * (threadsPerBlock / 32);
...
void train(NeuralNetwork* net,
	...
	// TODO: OCCUPANCY CHECKING HERE
	//size_t sharedMem = 256 * sizeof(float); // 1 KB shared memory
	///calculateOccupancyFORWARD(128, sharedMem, cudaFuncCachePreferShared);
	//calculateOccupancyBACKWARD(256, sharedMem, cudaFuncCachePreferShared);
    ...
    for (int epoch = 0; epoch < EPOCHS; epoch++) {
        ...
        for (int i = 0; i < numImages; i++) {
            ...
            // --- GPU Forward Pass ---
            forward_kernel<<<1, 128>>>(d_W1, d_W2, d_b1, d_b2, d_input, D_hidden, D_output);
